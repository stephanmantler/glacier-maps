...
*/
	 ...
	 var options201712 = optionsFromCapabilities(result, {
	     layer: 'treasure_20171211',
	     matrixSet: 'EPSG:3857',
	     requestEncoding: 'KVP'	     
	 });
	 var lmi_options = optionsFromCapabilities(result, {
	     layer: 'LMI_Kort',
	     matrixSet: 'EPSG:3857',
 	     requestEncoding: 'KVP'
	 });
	 
	 var lmiSource = new WMTS(/** @type {!olx.source.WMTSOptions} */ (lmi_options));
	 ...
	 var treasure201712Source = new WMTS(/** @type {!olx.source.WMTSOptions} */ (options201712));
	 
	 var treasureSource = new WMTS(/** @type {!olx.source.WMTSOptions} */ (options));
	 treasureSource.setAttributions('ice cave situation maps © <a href="http://www.hafjall.is/">Háfjall ehf.</a>')
	 
	 lmiSource.on('tileloadstart', function() { progress.addLoading(); });
	 lmiSource.on('tileloadend', function() { progress.addLoaded(); });
	 lmiSource.on('tileloaderror', function() { progress.addLoaded(); });
	 treasureSource.on('tileloadstart', function() { progress.addLoading(); });
	 treasureSource.on('tileloadend', function() { progress.addLoaded(); });
	 treasureSource.on('tileloaderror', function() { progress.addLoaded(); });
	 treasure201712Source.on('tileloadstart', function() { progress.addLoading(); });
	 treasure201712Source.on('tileloadend', function() { progress.addLoaded(); });
	 treasure201712Source.on('tileloaderror', function() { progress.addLoaded(); });
	 
	 var treasure_20171211 = new Tile({ opacity: 1, source: treasure201712Source, title:"Treasure Island 2017-12-11", visible:false })
	 var treasure_20180228 = new Tile({ opacity: 1, source: treasureSource, title:"Treasure Island 2018-02-28" })
	 ...
	 var vectorLayer = new ol.layer.Vector({
	     ...
	     style: style,
	     title: "Ice Cave Extents"
	 });
	 
	 map = new ol.Map({
	     layers: [
		 new ol.layer.Tile({ source: lmiSource, type: 'base', title:'LMI Map' }),
		 treasure_20180228,
		 treasure_20171211,
		 vectorLayer
	     ],
	 ...
	 function fixContentHeight(){
	     ...
	     var footer = $("div#foot");
	     var content = $("div#map");
	     var contentHeight = viewHeight - header.outerHeight() - footer.outerHeight() - 80;
